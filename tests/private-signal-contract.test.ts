...
describe("private repository signal contract", () => {
  ...
  test("documents private-local mode without changing the public collector", async () => {
    ...
    expect(collectorContract).toContain("it does not read private README text, so usage-guide detection remains conservative");
    ...
    expect(scoring).toContain("In public-only mode, Buildmarks can use public traces");
    expect(scoring).toContain("Names are redacted by default");
    expect(scoring).toContain("private file contents stay outside the built-in collector");
    expect(scoring).toContain("Private-local mode is an explicit owner-controlled exception");
