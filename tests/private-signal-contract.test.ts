...
describe("private repository signal contract", () => {
  ...
  test("allows only explicit redacted private-local signals", () => {
    ...
    expect(privateLocalRepositorySignalContract.disclosure.cardLabel).toBe("Public + Private Projects");
  ...
  test("documents private-local mode without changing the public collector", async () => {
    ...
    expect(privateContract).toContain("Do not commit private-local SVG, HTML, or JSON");
    expect(privateContract).toContain("fine-grained GitHub token");
    expect(privateContract).toContain("Public + Private Projects");
    expect(privateContract).toContain("cannot be checked independently on public GitHub");
    expect(privateContract).toContain("hide sensitive details");
    expect(privateContract).toContain("file contents");
    expect(privateContract).toContain("cannot tell whether that README contains useful setup instructions");
    expect(collectorContract).toContain("marks details it could not obtain instead of turning them into failures");
    expect(operations).toContain("cannot tell whether that README contains useful setup instructions");
    expect(privateContract).toContain("coarse repository shape");
    ...
    expect(collectorContract).toContain("`collectPublicGitHubProfile()` remains public-only");
    ...
    expect(scoring).toContain("Public cards cover public repositories");
    expect(scoring).toContain("Names are hidden by default");
    expect(scoring).toContain("private file contents stay outside the built-in collector");
    expect(scoring).toContain("Private-local mode is owner-controlled and opt-in");
    expect(readme).toContain("docs/private-repository-signal-contract.md");
    expect(readme).toContain("Private repositories are not part of `collectPublicGitHubProfile()`");
